...
def reproduce(paper_input, no_logging, no_checkpoints, max_iterations):
    ...
    try:
        ...
        # User Selection for Experiment Level
        if not no_logging: # Only prompt in interactive mode (using logging flag as proxy or just always prompt)
            ...
            selection = click.prompt("Please choose", type=click.Choice(['1', '2', '3']), default='1', show_default=True)
            ...
            if selection == '3':
                ...
                initial_state['custom_experiment_list'] = [e.strip() for e in custom_input.split(',')]
            
            click.echo(f"✅ Selected mode: {initial_state['experiment_selection_mode'].upper()}")

...
class OOMHandler:
    ...
    def _detect_gpu_memory(self) -> List[float]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(
                    f"🎮 Detected {len(memory_gb)} GPU(s): {[f'{gb:.1f}GB' for gb in memory_gb]}"
                )
                return memory_gb
    ...
    def extract_batch_size_params(self, script_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            batch_param_patterns = [
                ...
                (
                    r'--per[_-]?device[_-]?train[_-]?batch[_-]?size["\']?\s*[=:]\s*(\d+)',
                    "per_device_train_batch_size",
                ),
                (
                    r'--per[_-]?device[_-]?eval[_-]?batch[_-]?size["\']?\s*[=:]\s*(\d+)',
                    "per_device_eval_batch_size",
                ),
                (
                    r'--train[_-]?batch[_-]?size["\']?\s*[=:]\s*(\d+)',
                    "train_batch_size",
                ),
    ...
    def adjust_batch_size_in_script(
        self, script_path: str, reduction_factor: float = 0.5
        ...
        try:
            ...
            for param in param_info["patterns"]:
                ...
                content = content.replace(
                    old_match, new_match, 1
                )  # Replace first occurrence only

                adjustments.append(
                    {"param": param["param_name"], "old": old_value, "new": new_value}
                )
            ...
            if content != original_content:
                ...
                backup_path = Path(script_path).with_suffix(
                    Path(script_path).suffix + ".bak"
                )
    ...
    def suggest_batch_size(
        self, current_batch_size: int, gpu_memory_gb: float, model_size: str = "unknown"
    ) -> int:
        ...
        suggested = memory_rules.get(model_size, memory_rules["unknown"]).get(
            closest_tier, 8
        )
        ...
        if current_batch_size > 0:
            suggested = min(
                suggested,
                max(1, int(current_batch_size * self.batch_size_reduction_factor)),
            )
    ...
    def handle_oom(
        ...
        success, adjustment_info = self.adjust_batch_size_in_script(
            script_path,
            reduction_factor=self.batch_size_reduction_factor
            ** attempt,  # Reduce more on each attempt
        )
